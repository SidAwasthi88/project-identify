...
def run_camera_test():
    ...
    'q' is pressed. Every face in frame is matched independently, so multiple
    people at once is handled fine here (unlike enrollment, which requires
    exactly one person).

    Returns True if it ran, False if the camera or encodings couldn't load.
    """
    known_encodings, known_labels = _load_known_faces()

    if not known_encodings:
        print("No enrolled faces found. Enroll at least one student first.")
        return False
    ...
    while True:
        ...
        for (top, right, bottom, left), face_encoding, landmarks in zip(face_locations, face_encodings, landmarks_list):
            ...
            # Label strip above the box (wide enough for "Name (Roll No.)")
            label_width = max(right - left, 8 * len(label))
        ...
        # Small overlay showing how many faces are currently visible
        cv2.putText(
            frame, f"Faces in view: {len(face_locations)}", (10, frame.shape[0] - 15),
            cv2.FONT_HERSHEY_DUPLEX, 0.6, (255, 255, 255), 1
        )
